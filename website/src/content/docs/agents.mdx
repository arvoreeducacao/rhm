...
When you run `hub generate`, the CLI produces agent definitions from your config file (`hub.yaml` or `hub.config.ts`). The orchestrator agent reads the pipeline and delegates to sub-agents in order.
...
For tasks that benefit from parallel exploration and inter-agent communication, see [Agent Teams](/docs/agent-teams) — a coordination layer where multiple independent sessions share a task list and message each other directly.

> **Kiro note:** When generating for Kiro, agent definitions are placed in `.kiro/agents/` as sub-agent files with frontmatter (`name`, `description`, `tools`). The orchestrator delegates to each agent following the pipeline, just like Cursor and Claude Code.

> **OpenCode note:** OpenCode has native support for sub-agents via `@mention` and the Task tool. When generating for OpenCode, agents are placed in `.opencode/agents/` as markdown files with `mode: subagent` frontmatter. The orchestrator references agents using `@agent-name` syntax, matching OpenCode's native invocation model.
...
Pipelines are defined in your config under the `workflow` key:
