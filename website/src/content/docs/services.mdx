...
2. Makes the sandbox available to the relevant skills and subagents (e.g. `code-review`, `qa-testing`, and stack work), so they prefer running builds, tests, and browser automation inside the sandbox

This lets the agent run shell commands, execute code, control a browser, and read/write files inside the sandbox without touching the host machine.
