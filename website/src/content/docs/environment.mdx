...
Declare environment profiles in your config:
...
Each profile represents a target environment. Profiles are defined under `env.profiles` in your config:
...
## Extra Variables in .env.example

`hub generate` writes a `.env.example` at the hub root with every variable referenced as `${env:VAR}` by your MCPs, grouped by MCP name. Variables used outside MCPs — by scripts, hooks, or documented workflows — can be added with `env.example_extras`, grouped by a section name:
...
Each group renders as a `# <name>` comment followed by empty `VAR=` lines, after the MCP groups. Variables already emitted by an MCP group are deduplicated. Only names go into `.env.example` — values live in each developer's gitignored `.env`.
