...
- **Capabilities over ad-hoc** — Well-defined skills and tools beat improvised prompting
...
The compound effect is what matters. Every skill we write, every MCP we connect, every convention we capture — it benefits the entire team instantly. The setup gets better every week.
...
- **A library of skills** the AI pulls on demand — refine, review, test, and per-stack patterns
...
3. **Capabilities beat improvisation.** Ad-hoc prompting produces ad-hoc results. Giving the AI the right skills, tools, and context — and letting it pull what each task needs — produces consistent, high-quality output without a rigid script.

4. **The editor is the runtime.** We don't need another platform, another server, another dashboard. The AI editor is where the work happens. It should be where the capabilities live.
...
We want to standardize the skills, conventions, and tool connections that power our development — and make them tool-agnostic. Not tied to Cursor, not tied to Claude, not tied to any single vendor.

Repo Hub is the first step. The CLI, the config schema, the skill libraries, the MCP connections — all of it is designed to be adopted, extended, and contributed to by any team.
