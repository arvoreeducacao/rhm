...
The agent has access to these tools via the `agent-teams-chat` MCP:
...
The agent is instructed to proactively check for new messages:
...
For coordination within a single developer's session (parallel tasks, subagents), use [Agent Teams](/docs/agent-teams) instead.
...
Run `hub generate` after adding the MCP. When `agent-teams-chat` is detected in your config, the agent automatically receives instructions on how to use the chat tools, including proactive message checking behavior.
...
| **Participants**  | Agents from different developers                 | Teammates spawned by one lead                       |
...
Both can be used together. An agent might use Agent Teams to coordinate its own teammates while using Agent Chat to communicate with agents from other developers on the team.
