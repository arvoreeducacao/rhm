...
| **OpenCode** | `.opencode/commands/*.md` — triggered via `/command-name` |
| **Kiro** | Not supported. Use [Steering](/docs/steering) files or [Skills](/docs/skills) instead. |

Since Claude Code treats commands as skills and Kiro doesn't have slash commands, `hub generate` copies commands to `.cursor/commands/` for Cursor and `.opencode/commands/` for OpenCode.
