...
But context alone wasn't enough. We needed the AI to use the right **capabilities** for each task, not just answer questions. So we built the skills and tooling layer:

1. **One YAML file** declares everything — repos, services, MCPs, skills, conventions
2. **A CLI** generates editor configs from that YAML
3. **The editor becomes the runtime** — it exposes the connected tools and installed skills, and the AI pulls what each task needs

No daemon. No server. No platform. The AI editor does the work.
...
- A shared library of skills the AI pulls on demand for consistent output
...
The tools will keep changing. Editors, models, providers — none of it is permanent. What matters is the discipline: reusable skills, encoded knowledge, connected infrastructure, and human review at every step.

## Why we open-sourced it

We built Repo Hub to solve our own problem. But the problem isn't unique to Arvore.

Every team using AI coding assistants hits the same wall: context fragmentation, inconsistent results, manual overhead. The solution shouldn't be proprietary.
