...
This launches an interactive TUI that walks you through naming your workspace, choosing your AI editor, adding repositories, selecting agents and skills from the [directory registry](https://hub.arvore.com.br), picking MCP servers, and choosing your config format (YAML or TypeScript).

At the end, you'll have a fully configured workspace with your chosen config file (`hub.yaml` or `hub.config.ts`), agents, skills, and a `.gitignore`. See [Configuration](/docs/configuration) for details on both formats.
...
1. Reads your config file (`hub.yaml` or `hub.config.ts`) declaring the full pipeline
