...
Available helpers: `repo.nestjs()`, `repo.nextjs()`, `repo.react()`, `repo.elixir()`, `repo.go()`, `repo.python()`, `repo.custom()`. All accept an optional overrides object for commands, skills, env_file, etc. See [Configuration](/docs/configuration#typescript-config-hubconfigts) for details.
