...
|---------------|------------------|------------------|
| Linear MCP | Issue lifecycle automation | Create a ticket, link a PR, and move status as work progresses |
...
| SonarQube MCP | Static analysis feedback | Surface issues directly during work and link to PR findings |
...
Each helper pre-fills the correct `package` name. See [Configuration](/docs/configuration#typescript-config-hubconfigts) for the full list of MCP helpers.
...
When you run `hub generate`, these are written to `.cursor/mcp.json` (Cursor), `.mcp.json` (Claude Code), `.kiro/settings/mcp.json` (Kiro), or `opencode.json` (OpenCode), making their tools available to the agent.

## How Agents Use MCPs

The agent interacts with MCPs through tool calls — the connected tools appear in its tool schema automatically, no listing in the prompt required. For example:

- **Database MCP**: query the schema to understand table relationships before writing migrations
- **Datadog MCP**: search logs and traces to identify the root cause of a production error (often while using the `debugging` skill)
- **Playwright MCP**: navigate the web app, fill forms, and take screenshots to verify UI changes (often while using the `qa-testing` skill)
- **npm Registry MCP**: check package download counts and security signals before adding dependencies
