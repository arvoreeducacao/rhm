...
Unlike [sub-agents](/docs/agents), which run within a single session and can only report back to the main agent, teammates are fully independent sessions that can discover tasks, claim work, and message each other without going through the lead.
