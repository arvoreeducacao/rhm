...
As your workspace grows, new repos get cloned but not always added to `hub.yaml`. Skills, agents, and steering files get created inside editor-specific folders but never promoted to the canonical `skills/`, `agents/`, or `steering/` directories. MCP servers get added directly to editor configs without being declared in `hub.yaml`. The `hub scan` command finds all of them for you.
...
Walks the workspace root looking for directories that contain a `.git` folder but aren't listed in `hub.yaml`. For each one it:
...
Scans editor-specific MCP config files for servers not declared in `hub.yaml`:
...
MCP servers found in editor configs but missing from `hub.yaml` are reported so you can add them manually (since MCPs require `package`, `url`, or `image` fields that can't be inferred).
