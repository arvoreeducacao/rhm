...
As your workspace grows, new repos get cloned but not always added to `hub.yaml`. Skills, agents, and steering files get created inside editor-specific folders but never promoted to the canonical `skills/`, `agents/`, or `steering/` directories. The `hub scan` command finds all of them for you.

## What it does

`hub scan` performs two passes:
