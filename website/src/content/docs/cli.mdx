...
```bash
hub generate --editor pi           # Pi (registers hub-pi extension; runtime, no files)
...
Produces:
- **Pi**: no generated files. Registers `@arvoretech/hub-pi` in `.pi/settings.json` (merge-preserving existing packages/skills) and points the skills dir. The extension derives the capabilities prompt, skills, and MCP wiring from the config at runtime.
