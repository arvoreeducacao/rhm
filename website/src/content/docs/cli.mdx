...
2. Choosing your AI editor (Pi, Cursor, Kiro, Claude Code, OpenCode)
3. Adding repositories with tech stack detection
4. Selecting skills from the [directory registry](https://hub.arvore.com.br)
5. Picking MCP servers for infrastructure access
6. Choosing config format (YAML or TypeScript)

Creates a directory with the config file (`hub.yaml` or `hub.config.ts`), `.gitignore`, `README.md`, and installs selected skills from the registry.

### `hub setup`

Full workspace setup — clone repos, start services, install tools, generate env files, install dependencies, and install declared skills:
...
```bash
hub generate --editor pi           # Pi (AGENTS.md + steering, registers hub-pi extension)
hub generate --editor cursor       # Cursor IDE
...
| Flag | Description |
|------|-------------|
| `-e, --editor <editor>` | Target editor (pi, cursor, claude-code, kiro, opencode) |
...
Produces:
- **Pi**: `AGENTS.md` (capabilities prompt with steering appended) read natively by Pi at startup. Registers `@arvoretech/hub-pi` in `.pi/settings.json` (merge-preserving existing packages/skills) and points the skills dir. The extension adds MCP wiring, repo tools, persona, and hooks at runtime. Skip the `AGENTS.md` generation by setting `pi.injectCapabilities: false`.
- **Cursor**: `.cursor/rules/orchestrator.mdc`, `.cursor/rules/<steering>.mdc`, `.cursor/skills/`, `.cursor/skills/hub-docs/`, `.cursor/commands/`, `.cursor/hooks.json`, `.cursor/mcp.json`, `<name>.code-workspace`, `.gitignore`, `.cursorignore`
- **Claude Code**: `CLAUDE.md` (with steering appended), `.claude/skills/`, `.claude/skills/hub-docs/`, `.claude/settings.json` (with hooks), `.mcp.json`, `.gitignore`
- **Kiro**: `.kiro/steering/orchestrator.md`, `.kiro/steering/<steering>.md`, `.kiro/skills/`, `.kiro/skills/hub-docs/`, `.kiro/settings/mcp.json`, `AGENTS.md`, `<name>.code-workspace`, `.gitignore`
- **OpenCode**: `opencode.json`, `.opencode/rules/orchestrator.md`, `.opencode/rules/<steering>.md`, `.opencode/skills/`, `.opencode/skills/hub-docs/`, `.opencode/commands/`, `.opencode/plugins/hub-hooks.js`, `.gitignore`
...
```bash
hub skills sync                                            # Install all skills declared in the config
...
> **Deprecated: `hub agents`.** Agents were replaced by skills in the skills-centered model. The command now prints a migration notice and exits. Use `hub skills` instead, and declare capabilities under `skills:` in your config.
...
See [Persona](/docs/persona) for full documentation.
